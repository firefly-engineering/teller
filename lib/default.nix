# Teller library functions — versioned toolchain registry helpers
#
# These functions are system-independent (no pkgs dependency at top level).
# mkMetaPackage takes pkgs as a parameter when called.
{
  lib,
  currentTime ? null,
}:
...
in
{
  ...
  #
  # Usage:
  #   go = teller.lib.resolveTool registry "go" { version = "1.22"; };
  #   rust = teller.lib.resolveTool registry "rust" {};  # uses default
  resolveTool =
    registry: name: spec:
    let
  ...
  #
  # Usage:
  #   packages = teller.lib.resolveToolchains registry toolchainDeclaration;
  resolveToolchains =
